...
Estimar el valor de: $$I =
  \int_{0}^{1}  \int_{0}^{1} \frac{1}{3 +  \cos( x) \sin(y) }    \,dx   \,dy$$
...
Es decir, \(I_{6} = \mathbb{E}[g(\vec{U})]\), donde \(\vec{U} = \big(U_{1}, U_{2}\big)\) es un vector aleatorio uniforme en \((0, 1) \times (0, 1)\) (equivalentemente, \(U_{1}\) y \(U_{2}\) son variables independientes uniformes en el intervalo \((0, 1)\)) y \(g(u_{1}, u_{2}) = \frac{1}{3 + \cos{x} + \sin{y}}\).
...
```{r generacion-2}
# Creamos una función para generar números aleatorios (vectores de loongitud 2).
